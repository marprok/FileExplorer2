...
namespace utils
{
    ...
    private:
        ...
        std::size_t m_limit;
    public:
        SCRVector(std::size_t base, std::size_t window_size, std::vector<T> & data);
    ...
    template<typename T>
    SCRVector<T>::SCRVector(std::size_t base, std::size_t window_size, std::vector<T> & data)
        :m_data(data),
         m_base(base),
    ...
    template<typename T>
    int SCRVector<T>::operator[](size_t index)
    {
        /* No bounds checking */
